...
function parseTextFallback(textInput) {
	return textInput.split(/\r?\n/).map((line) => line.trim()).filter((line) => line.length > 0).map((line) => {
		...
		if (notes.toUpperCase().includes("PUTAWAY") || notes.toUpperCase().includes("ZASKLADNENI")) departmentId = "putaway";
		if (notes.toUpperCase().includes("OBWF") || notes.toUpperCase().includes("WATERFRONT")) departmentId = "obwf";
		if (isVNA) departmentId = "vna";
		if (notes.toUpperCase().includes("OBWI") || notes.toUpperCase().includes("WEB")) departmentId = "obwi";
		return {
...
var extractOperatorsFn = createServerFn({ method: "POST" }).validator((d) => d).handler(extractOperatorsFn_createServerFn_handler, async ({ data }) => {
	...
	const apiUrl = openaiKey ? "https://api.openai.com/v1/chat/completions" : "https://ai.gateway.lovable.dev/v1/chat/completions";
	...
	try {
		...
		if (!aiResponse.ok) {
			...
			if (textInput) return { operators: parseTextFallback(textInput) };
			const message = aiResponse.status === 429 ? "Rozpoznávání je momentálně zahlcené, zkuste to prosím za chvíli znovu." : aiResponse.status === 402 || aiResponse.status === 401 ? "Chyba ověření nebo vyčerpaný kredit pro AI. Zadejte prosím jména textem." : "Rozpoznávání z fotky se nezdařilo (Chyba AI). Zkuste to znovu, nebo zadejte jména textem.";
		...
		else throw new Error(error?.message || "Při zpracování snímku došlo k chybě (výpadek AI služby).");
	}
